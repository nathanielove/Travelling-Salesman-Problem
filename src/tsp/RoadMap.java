...
public class RoadMap {
	...
	private HashMap<Pair, Double> map;
	 ...
	 */
	public RoadMap(String[] cities, double[][] dists){
		...
		for(int i = 0; i < dists.length - 1; ++i){
			for(int j = i + 1; j < dists.length; ++j){
				this.map.put(new Pair(cities[i], cities[j]), dists[i][j]);
			}
		}
	}
	 ...
	 */
	public RoadMap(String[] cities, Map<Pair, Double> map){
	...
	public double getDistance(String a, String b){
		this.checkCity(a);
		this.checkCity(b);
		return this.map.get(new Pair(a, b));
	}
	...
	private void initialize(){
		...
		this.map = new HashMap<Pair, Double>();
	}
